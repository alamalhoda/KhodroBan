...
- **Merge** → ساده‌تر، تاریخچه نشان می‌دهد که فیچر از چه زمانی با develop هماهنگ شده
- **Rebase** → تاریخچه خطی‌تر و تمیزتر می‌شود، اما اگر برنچ را قبلاً با دیگران share کرده‌اید، خطرناک است

### * از merge استفاده کن اگر branch قبلاً با دیگران share شده باشد
* از rebase استفاده کن اگر branch فقط در اختیار خودت است و تاریخچه تمیز می‌خواهی

5. همه commitها را دوباره چک کن و تست بزن

### مرحله ۵ – Push کردن برنچ به‌روزشده

قبل از push کردن، حتماً یک بار `git status` بزنید و مطمئن شوید که در وضعیت تمیز (nothing to commit) هستید.
...
*  `--force-with-lease` ایمن‌تر از `--force` است و فقط وقتی push می‌کند که کسی دیگر روی برنچ تو تغییر نداده باشد.
* هرگز از git push --force استفاده نکن مگر اینکه کاملاً مطمئن باشی هیچ‌کس دیگری روی این branch کار نکرده است.
...
بعد از merge موفق، شاخه develop روی سرور به‌روز شده است. حالا همه اعضای تیم می‌توانند با `git pull` آخرین تغییرات (شامل فیچر شما) را دریافت کنند.
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
