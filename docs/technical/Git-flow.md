...
در اینجا دو روش رایج برای ادغام وجود دارد. هر دو روش قابل استفاده هستند، اما بسته به شرایط پروژه (مانند اندازه تیم، نیاز به تاریخچه تمیز یا حفظ کامیت‌ها) یکی را انتخاب کنید. روش اول (به‌روزرسانی فیچر قبل از PR) برای تیم‌های بزرگ‌تر و پروژه‌های پیچیده‌تر توصیه می‌شود زیرا امکان حل کانفلیکت‌ها قبل از ادغام نهایی را فراهم می‌کند و فرآیند code review را بهبود می‌بخشد. روش دوم (squash مستقیم روی develop) برای پروژه‌های شخصی یا کوچک‌تر مناسب‌تر است زیرا ساده‌تر و سریع‌تر است.

##### **روش ۱: به‌روزرسانی فیچر قبل از PR (توصیه‌شده برای یادگیری اصول حرفه‌ای)**

این روش برنچ فیچر را قبل از ایجاد Pull Request با آخرین تغییرات develop همگام می‌کند. این کار کانفلیکت‌ها را زودتر حل می‌کند، امکان تست محلی را فراهم می‌آورد و PR را تمیزتر می‌کند.

```bash
# ۱. برو به برنچ فیچر
git checkout feature/user-authentication

# ۲. آخرین تغییرات develop را fetch کن و merge کن (یا rebase برای تاریخچه خطی)
git fetch origin
git merge origin/develop  # یا git rebase origin/develop برای تاریخچه خطی (احتیاط: اگر برنچ share شده، از --force-with-lease استفاده کن)

# اگر کانفلیکت داشت:
# - فایل‌های مربوطه را دستی ویرایش کن
# - git add <فایل‌ها>
# - git merge --continue  # یا git rebase --continue

# ۳. تست کن که پروژه هنوز درست کار می‌کند (اجرا کن تست‌ها، build کن پروژه)

# ۴. تغییرات به‌روزرسانی‌شده را push کن
git push origin feature/user-authentication  # اگر rebase کردی: git push --force-with-lease

# ۵. ایجاد Pull Request به develop از GitHub/GitLab
# - در PR، گزینه Squash and Merge یا Create a merge commit را انتخاب کن (Squash برای تاریخچه تمیز توصیه می‌شود)
# - بعد از approve شدن PR، merge را انجام بده
```

**مزایا این روش:**
- کانفلیکت‌ها محلی حل می‌شوند (نه روی سرور PR)
- فیچر قبل از ادغام با develop هماهنگ است
- امکان self-review یا code review قبل از ادغام نهایی
- تاریخچه develop تمیزتر می‌ماند

##### **روش ۲: Squash مستقیم روی develop (روش ساده‌تر برای پروژه‌های شخصی)**
...
#### **جدول مقایسه دو روش ادغام feature به develop**

| جنبه مقایسه                  | روش ۱: به‌روزرسانی فیچر قبل از PR (merge/rebase روی فیچر) | روش ۲: Squash مستقیم روی develop |
| ----------------------------- | ------------------------------------------------------------- | --------------------------------- |
| **محل حل کانفلیکت**         | روی برنچ فیچر (محلی، قبل از PR)                              | روی برنچ develop (بعد از merge)  |
| **امکان تست قبل از ادغام** | بله (تست محلی بعد از merge/rebase)                           | خیر (تست بعد از commit نهایی)   |
| **تاریخچه گیت**              | می‌تواند خطی (با rebase) یا با merge commit باشد             | همیشه یک کامیت تمیز (squash)    |
| **مناسب برای code review**   | عالی (PR بعد از به‌روزرسانی فیچر)                          | متوسط (PR بدون به‌روزرسانی ممکن است کانفلیکت ایجاد کند) |
| **سرعت اجرا**                | کمی طولانی‌تر (نیاز به push اضافی)                          | سریع‌تر (مستقیم روی develop)     |
| **ریسک**                     | کمتر (کانفلیکت‌ها زود حل می‌شوند)                          | بیشتر (کانفلیکت روی develop ممکن است deploy را مختل کند) |
| **مناسب برای**               | تیم‌های بزرگ، پروژه‌های پیچیده، یادگیری اصول حرفه‌ای      | پروژه‌های شخصی، کوچک، سریع       |
...
با پیروی از این راهنمای کامل Git Flow، پروژه KhodroBan می‌تواند **پایداری production** را حفظ کند در حالی که **توسعه فعال** ادامه دارد. این مدل به ویژه برای پروژه‌هایی که **stability** اولویت اول است، ایده‌آل می‌باشد.ر